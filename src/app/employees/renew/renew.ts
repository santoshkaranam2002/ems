...
export class Renew {
  ...
  pill = pillClass;
  isDisabled(r: Row): boolean { return r['status'] === 'Renewed' || r['status'] === 'Cancelled'; }
  ...
  confirmAction() {
    ...
    this.target['status'] = 'Renewed';
    if (this.form['newExpiry']) {
      const d = new Date(this.form['newExpiry']);
      this.target['passExpiry'] = d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
    }
